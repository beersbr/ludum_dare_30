...
function Player(args){
	...
	this.onCollide = function(o){
		...
		if(o instanceof DragonBoss){
			...
			lifeLost = 3 * Game.difficulty
			this.health -= lifeLost;
			StatusBar.removeHealth({amount:lifeLost});
...
function GameLevel(level){
	...
	this.generateLevelImage = function(){
		...
		$.ajax({url:"json/"+this.level,dataType:"json", async:false,success:function(mapJson){
			...
			if(Game.player && (Game.currentLevel != 0 || Game.difficulty != 1)){
...
var Game = (function(){
	...
	game.nextLevel = function(){
		...
		// If we are on the last level, reset the level counter and increase the difficulty
